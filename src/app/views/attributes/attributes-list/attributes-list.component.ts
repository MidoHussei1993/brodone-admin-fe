...
export class AttributesListComponent extends ListComponent<any, any>
implements OnInit
{
...
getList() {
  this.spinner.show();
  this.attributesService.get(this.route.snapshot.params.restaurantId,this.filter).subscribe(
    (res: any) => {
      this.spinner.hide();
      this.list = res.data;
...
deleteItem(item){
  this.spinner.show();
  this.attributesService.delete(this.route.snapshot.params.restaurantId,item.id).subscribe(
    (res: any) => {
      this.getList();
      this.spinner.hide();
    },
